...
import (
	...
	"github.com/rohanshrestha09/dev-synapse/configs"
	"github.com/rohanshrestha09/dev-synapse/scopes"
	"gorm.io/gorm"
)
...
func Delete[T any](data T) (func(string) Response, *Error) {
	...
	if err := configs.DB.Delete(&data).Error; err != nil {
		return nil, &Error{Response{err.Error()}, err}
	}
...
func RecordExists[T any](filter T) (bool, error) {

	var data T

	err := configs.DB.Where(&filter).First(&data).Error
